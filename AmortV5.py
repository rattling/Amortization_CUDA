...
@guvectorize(['void( int64[:], int64[:], intp[:],intp[:], int64[:])'], '(n),(n),(),()->(n)',  target='cuda')
def cumVestWrapperGU(shares, vals,rowCount, numTranches, cumVest):      
    total=0
    # GET CUMULATIVE VESTMENT AMOUNT PER GRANT FOR EACH TRANCHE
    for j in range(0,numTranches[0]+1):            
        total+=shares[j]*vals[j]
        cumVest[j]=total      
...
def runCode(numba_flag, scaling):
    ...
    if numba_flag == 1:
        #Calculate cumulative vestments for each tranche using jit
        func_start = timer()    
        cumVest=cumVestWrapperN(rowCount, numTranches, shares,vals, cumVest)
        timing=timer()-func_start
        print("Function: cumVestN duration (seconds):" + str(timing))
        
        #Calculate cumulative vestments for each tranche using guvectorize
        func_start = timer()    
        cumVest=cumVestWrapperGU(shares,vals,rowCount, numTranches, cumVest)
        timing=timer()-func_start
        print("Function: cumVestN duration (seconds):" + str(timing))
        
        #Allocate a segment number to each tranche using jit
        func_start = timer()   
